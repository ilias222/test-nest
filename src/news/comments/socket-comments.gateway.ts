import { SubscribeMessage, WebSocketGateway, OnGatewayInit, WebSocketServer,
    ...
    async handleMessage(client: Socket, comment: Comment, comments):Promise<void> {
    const { idNews, message } = comment;
    // Извлекаем объект пользователя, который установлен в ws-jwt.guard.ts
    const userId: number = client.data.user.id;
    // Создаём комментарий
    const _comment = await this.commentsService.create(idNews, message, userId, comments.path);
